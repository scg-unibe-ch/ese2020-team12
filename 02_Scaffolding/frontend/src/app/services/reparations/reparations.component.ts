...
export class ReparationsComponent implements OnInit {
  ...
  servSendFilter(): void {
    ...
    this.httpClient.get(environment.endpointURL + 'filter/provided-service/1?city=' + this.servLoc + '&lowerPrice=' + this.servLowerPrice + '&upperPrice=' + this.servUpperPrice + '&servicePer=' + this.radioFilter(this.servicePer))
      .subscribe(items => {
